...
from analysis.activity_gate import activity_gate
    ...
    if not market_session.is_live:
        ...
        if decision.decision_confidence < CONFIG.decision_min_core_confidence:
            blockers.append(
                ...
                f"{CONFIG.decision_min_core_confidence:.0f}%"
            )
        activity_blocked, activity_note = activity_gate(setup, big_player)
        if activity_note:
            (blockers if activity_blocked else reasons).append(activity_note)
